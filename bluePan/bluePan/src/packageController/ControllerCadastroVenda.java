...
public class ControllerCadastroVenda implements Initializable {
	...
	void btnPesquisarAction(ActionEvent event) {
		ArrayProduto = FXCollections.observableArrayList(produtoDAO.search(TxtFieldPsquisa.getText()));

		columnID.setCellValueFactory(new PropertyValueFactory<>("Codigo"));
		columnNome.setCellValueFactory(new PropertyValueFactory<>("Nome"));
		columnPreco.setCellValueFactory(new PropertyValueFactory<>("Preco_un"));
    ...
    void definirCliente(KeyEvent event) {
    	if(clienteDAO.search(txtCPFCliente.getText()) != null) {
    		...
    		cliente2 = cliente1.get(0);		//OBS:colocar esse metodo no "Mouse" |One Kay Type
    	...
    	@FXML
        void definirVendedor(KeyEvent event) {
        	if(vendedorDAO.search(txtCPFVendedor.getText()) != null) {
        		vendedor1 = vendedorDAO.search(txtCPFVendedor.getText());
        		vendedor2 = vendedor1.get(0);		//OBS:colocar esse metodo no "Mouse" |One Kay Type
        		txtIDVendedor.setText(vendedor2.getId_Vendedor());
        		txtNomeVendedor.setText(vendedor2.getNome());
        	}
    } 	
   }